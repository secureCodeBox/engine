...
public class DefectDojoPersistenceProviderTest {
    ...
    @Mock
    DefectDojoToolService defectDojoToolService;

    @Mock
    DefectDojoProductService defectDojoProductService;

    @Mock
    DefectDojoEngagementService defectDojoEngagementService;

    @Mock
    DefectDojoFindingService defectDojoFindingService;
    ...
    public void setUp() {
        ...
        defectDojoUserService.defectDojoUrl = "http://localhost:8000";

        DefectDojoResponse<ToolType> responseExisting = new DefectDojoResponse<>();
        responseExisting.setCount(1);
        when(defectDojoToolService.getToolTypeByName(any())).thenReturn(responseExisting);

        EngagementResponse engagementResponse = new EngagementResponse();
        engagementResponse.setId(2);
        when(defectDojoEngagementService.createEngagement(any())).thenReturn(engagementResponse);
        when(defectDojoProductService.getProductId("Nmap Scan 11")).thenReturn(1l);
        when(defectDojoProductService.getProductId("Nonexisting")).thenThrow(DefectDojoProductNotFound.class);


        metaData = new HashMap<>();
        metaData.put(DefectDojoMetaFields.DEFECT_DOJO_USER.name(), "John Doe");
        when(defectDojoUserService.getUserId(eq("John Doe"))).thenReturn(5l);

        report = new Report();
        report.setRawFindings("\"[]\"");
        report.setFindings(Collections.emptyList());

        when(defectDojoToolService.retrieveOrCreateToolConfiguration(eq("http://crazy.buildserver"), eq("Build Server"))).thenReturn(5l);
        when(defectDojoToolService.retrieveOrCreateToolConfiguration(eq("http://crazy.scm_server"), eq("Git Server"))).thenReturn(7l);
        when(defectDojoToolService.retrieveOrCreateToolConfiguration(eq("https://github.com/secureCodeBox"), eq("Security Test Orchestration Engine"))).thenReturn(9l);
...
//        when(defectDojoToolService.getToolTypeByName("Git Server")).thenReturn(responseEmpty);
//
...
//
//        verify(defectDojoToolService, times(1)).createToolType(eq("Git Server"), any());
//        verify(defectDojoToolService, times(0)).createToolType(eq("Build Server"), any());
//        verify(defectDojoToolService, times(0)).createToolType(eq("Security Test Orchestration Engine"), any());
//    }
...
//        persistenceProvider.persist(securityTest);
//
//        verify(defectDojoToolService, times(0)).createToolType(any(), any());
//    }
...
//        persistenceProvider.persist(securityTest);
//
//        verify(defectDojoEngagementService, times(1)).createEngagement(
//            eq(payload)
//        );
//    }
//
...
//
//        persistenceProvider.persist(securityTest);
//        verify(defectDojoFindingService, times(1)).createFindings(
//                eq("<Some Xml stuff>\n<Don't know how the form is>"),
//                eq(2l),
//                eq(5l),
//                eq("2019-01-07"),
...
//    }
    ...
    public void createProduct() {
        ...
        defectDojoProductService.createProduct(productName);
        verify(defectDojoProductService, times(1)).createProduct(
            eq(productName)
        );
    }
}
